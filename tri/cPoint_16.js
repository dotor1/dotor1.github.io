...
class cPoint{
	...
	cb_screenshot(p_ctx , p_imgcs_scale){
		let Ox_vb     = this.sePoint.getAttribute('cx');
		let Oy_vb     = this.sePoint.getAttribute('cy');
		let radius_vb = this.sePoint.getAttribute('r');
		
		let O_imgcs      = this.oSCR.vbcs2imgcs( Ox_vb, Oy_vb, p_imgcs_scale);	//원의 중점
		
		//let radius_imgcs = this.oSCR.vbcs2imgcs( radius_vb, 0, p_imgcs_scale);	//거리는 그냥 이렇게 하기로 함
		let radius_imgcs = this.oSCR.dist_vbcs2imgcs( radius_vb, p_imgcs_scale);	//거리는 그냥 이렇게 하기로 함
		...
		let Ox = O_imgcs.x;
		let Oy = O_imgcs.y;
		//let radius = radius_imgcs.x;
		let radius = radius_imgcs;
		let start_radian = 0;
		let end_radian   = 2*Math.PI;
		let dir = CW;

		//선 스타일
		p_ctx.strokeStyle = this.oSCR.get_style('.c_points','stroke');
		p_ctx.fillStyle = this.oSCR.get_style('.c_points','fill');
...
//		</g>		
	...
	//{{2022.08.12. screenshot call-back에서 이용
	vb_x = 0;
	vb_y = 0;
	angle_degree = 0;
	//}}
...
//		</g>		
	...
	cb_screenshot(p_ctx , p_imgcs_scale){
		//debugger;
		let r     = this.oSCR.oConfig.seStyle['point_symbol_delta'];
		let theta = this.oSCR.calc_deg2rad( -this.angle_degree );

		let symbol_vbc  = this.oSCR.calc_CircleXY2(this.vb_x, this.vb_y , r , theta);
		let symbol_imgc = this.oSCR.vbcs2imgcs( symbol_vbc.x , symbol_vbc.y , p_imgcs_scale);
		
		//
		//debugger;
		let font_family = this.oSCR.get_style('.c_symbol_text','font-family');	//선 두께	
		let font_size = this.oSCR.oConfig.seStyle['point_symbol_font_size'];
		//String( this.oSCR.oConfig.seStyle['point_symbol_font_size'] ) + 'px';		
		...
		p_ctx.fillText(this.symbol,symbol_imgc.x,symbol_imgc.y);		
	}
}	